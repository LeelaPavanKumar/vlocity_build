...
Vlocity.compare = async function(options) {
    var comparisonInfo = Vlocity.getComparisonInfo(options);
    if(options.target.includes('_VersionCheck')){
        var initialManifest = options.manifest[0];
        options.maxDepth = 0;
    }
    var refreshPromises = [];
    ...
    if (options.refreshSource) { 
        ...
        if (options.source != 'local') {
            ...
            if(initialManifest){
                refreshOptionsSource.manifest = initialManifest.SourceManifest;
            }
    ...
    if (options.refreshTarget) {
        ...
        if (options.target != 'local') {
            var refreshOptionsTarget = Vlocity.getEnvironmentInfo(options.target.replace('_VersionCheck',''), options);
            if(initialManifest){
                refreshOptionsTarget.manifest = initialManifest.TargetManifest;
                refreshOptionsTarget.target = refreshOptionsTarget.target.replace('_VersionCheck','');
            }
    ...
    if (!fs.existsSync(comparisonInfo.targetFileCurrent) || options.resetDiffs) {
        ...
        options.targetData = await Vlocity.getDataPacks(Vlocity.getEnvironmentInfo(options.target.replace('_VersionCheck',''), options));
