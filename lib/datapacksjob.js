...
DataPacksJob.prototype.runJob = async function(action, jobData) {
    ...
    try {
        ...
        let updatedQueryInfo = this.vlocity.utilityservice.filterForVersionQueries(this.queryDefinitions, jobInfo);
        this.queryDefinitions = updatedQueryInfo.queryDefinitions;
        jobInfo = JSON.parse(JSON.stringify(updatedQueryInfo.jobInfo)); 
        ...
        if (jobInfo.queries && !jobInfo.versionExport) {
...
DataPacksJob.prototype.formatManifest = async function(jobInfo) {
    ...
    if (jobInfo.gitCheck && jobInfo.jobAction == 'Deploy') {
        ...
        if(!jobInfo.versionExport && !jobInfo.versionCompare){
